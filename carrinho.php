								...
								<!-- Content -->
									<div id="content">
										<article class="box post">
											<header class="style1">
												<h2>Seu Carrinho</p><hr />
											</header>

<? 
$soma=0;
    $query = mysql_query("SELECT * FROM carrinho where id_idusuario='$idu'");
while ($row = mysql_fetch_array($query)) {
...
/*consulta produto*/
$cmdsql = "select * from produto where idproduto='{$prod}'";
$resultado = mysqli_query($conexao, $cmdsql);
$pesquisa = mysqli_fetch_assoc($resultado);
$idp=$pesquisa["idproduto"];
$produto = $pesquisa["nome"];
$foto = $pesquisa["foto"];
    ...
    echo "<div>
    <form method='POST' class='' action='excluir-car.php'>
    <input type='hidden' value='".$t."' name='total'>
    <input type='hidden' name='id' value='".$idp."'>
    <input type='hidden' name='idu' value='".$idu."'>
    <img width='100' src='data:image/jpeg;base64,".base64_encode($foto) ."'><br><input style='float:right;' type='submit' value='Excluir' name='excluir' class=''><br>
    <b>Produto: </b>".$produto."<br>
    <b>Valor: </b>R$ ".$t."<br></form>

    <form  method='POST' class='' action='compra.php'>
    <input type='hidden' value='".$t."' name='total'>
    <input type='hidden' name='id' value='".$idp."'>
    <input type='hidden' name='idu' value='".$idu."'>
    <input style='float:right;' type='submit' value='Comprar unidade' name='comprar' class='button style1'>
    </form>

    </div><BR><BR><hr />";
   
}
 echo "<form method='POST' class='' action='compra.php'>
 <b style='float:left; width:200px'>Total do carrinho: </b>
<input type='text' readonly='true' style='float:left;width:100px;' value='R$ ".$soma."' name='total'>
 <div style='float:right'>
    <input type='hidden' name='idu' value='".$idu."'>
 <input style='float:right;' type='submit' value='Comprar todos' name='comprar' class='button style1'>
 </div></form><BR><BR><BR>";
?>
