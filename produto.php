...
$query = mysql_query("SELECT * FROM `produto` where `idproduto` = '$id'");
while ($row = mysql_fetch_array($query)) {
$id = $row['0'];
$nome = $row['1'];
$desc = $row['2'];
...
<form method='POST' class='' action='cad-car.php' id='form-car'>
";
          if (usuarioQueEstaLogado()) {
           echo " <input type='hiden' value='".$idu."' name='idu' style='display:none;'>";
             }
...
echo "
        ...
        <img width='100' src='data:image/jpeg;base64,".base64_encode($foto) ."'>
         <div> <b>Nome do produto: </b>".$nome." <br> <b>Tipo: </b>".$ti."</div><button style='float:right' type='submit' class='button style1' name='cadastrar'>Adicionar ao carrinho</button>
