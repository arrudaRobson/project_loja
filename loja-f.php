								...
								<!-- Content -->
									<div id="content">
										<article class="box post">
											<header class="style1">
												<h2><? if ($_POST) {$idd=$_POST['idd'];$query = mysql_query("SELECT * FROM tipo WHERE idtipo='$idd'");while ($row = mysql_fetch_array($query)) {$id = $row['0'];$nome = $row['1'];echo $nome;}}?></p><hr style="margin-top:-50px;" />
											</header>
...
$idd=$_POST['idd'];
$query = mysql_query("SELECT * FROM produto WHERE tipo='$idd'");
while ($row = mysql_fetch_array($query)) {
$id = $row['0'];
$nome = $row['1'];
$desc = $row['2'];
...
$qtd = $row['6'];
$foto = $row['7'];
echo "<div class='produto' style='background:; width:350px;float:left; padding-left:50px;'><hr size='250px' width='1' style='float:right' color='gray'>
